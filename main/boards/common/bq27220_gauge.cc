...
bool Bq27220Gauge::GetBatteryLevel(int& level, bool& charging, bool& discharging) {
    ...
    // Voltage and current are separate I2C reads. A transient current-read
    // failure must not manufacture a charging -> idle -> charging edge for
    // callers that successfully received the battery level.
    charging = current_direction_initialized_ && last_charging_;
    discharging = current_direction_initialized_ && last_discharging_;
