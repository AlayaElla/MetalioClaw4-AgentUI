...
public:
    ...
    bool Mount() {
        ...
        ConfigureSharedHostedController(host);
...
private:
    ...
    // ESP-IDF 6 lets ESP-Hosted create the single SDMMC controller before
    // app_main. The SD card shares that controller on Slot 0, while the C5
    // coprocessor uses Slot 1. Re-running host.init() would try to allocate a
    // second controller and fail with ESP_ERR_NOT_FOUND. Keep deinit_p intact
    // so unmounting still releases only Slot 0 without disturbing Hosted.
    static esp_err_t SharedHostedControllerAlreadyInitialized() {
        return ESP_OK;
    }

    static void ConfigureSharedHostedController(sdmmc_host_t& host) {
#if CONFIG_ESP_HOSTED_SDIO_HOST_INTERFACE && \
    (ESP_IDF_VERSION >= ESP_IDF_VERSION_VAL(6, 0, 0))
        host.init = &SharedHostedControllerAlreadyInitialized;
#else
        (void)host;
#endif
    }
