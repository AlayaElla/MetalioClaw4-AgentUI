...
public:
    ...
    // Register a device-level callback that remains active while feature pages
    // attach and detach their own temporary callback.  The Bluetooth module
    // sends state changes outside the settings page, so dropping those bytes
    // would make command acknowledgement and connection state unreliable.
    void registerSystemCallback(
        std::function<void(const std::vector<uint8_t>&)> callback) {
        std::lock_guard<std::mutex> lock(m_callbackMutex);
        m_systemCallback = std::move(callback);
    }
...
private:
    ...
    static void uartReceiveTask(void* arg) {
        ...
        while (true) {
            ...
            if (length > 0) {
                ...
                {
                    std::lock_guard<std::mutex> lock(uart->m_callbackMutex);
                    system_callback = uart->m_systemCallback;
                    page_callback = uart->m_callback;
                }
                if (system_callback) {
    ...
    std::function<void(const std::vector<uint8_t>&)> m_systemCallback;
};
