        ...
        if (!in_light_sleep_mode_) {
            ...
            app.Schedule([this, &app]() {
                while (in_light_sleep_mode_) {
                    ...
                    ESP_ERROR_CHECK(esp_lv_adapter_refresh_now(nullptr));
                    ESP_ERROR_CHECK(esp_lv_adapter_pause(-1));
                    ...
                    ESP_ERROR_CHECK(esp_lv_adapter_resume());
                    ...
                    if ((wakeup_causes & BIT(ESP_SLEEP_WAKEUP_TIMER)) == 0) {
                        break;
                    }
                }
                WakeUp();
