...
esp_err_t UartEthModem::RunNormalModeInitSequence() {
    ...
    if (!WaitForRegistration(60000)) {
        if (cell_info_.stat == 3) {
            ESP_LOGE(kTag, "Registration denied; keeping AT channel alive");
        } else {
            ESP_LOGE(kTag, "Registration timeout; keeping AT channel alive");
        }
        // Network data cannot start, but SIM-slot switching still depends on
        // this modem instance and its AT worker tasks.  Treat it like flight
        // mode instead of tearing the modem down.
        initialized_ = true;
        SetNetworkEvent(UartEthModemEvent::InFlightMode);
        return ESP_OK;
    }
