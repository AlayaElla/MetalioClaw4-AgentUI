...
private:
    ...
    esp_err_t beginImpl(i2c_master_bus_handle_t i2c_bus, uint32_t dev_addr) {
        ...
        esp_err_t ret = metalio_tca9555_new(
            i2c_bus, dev_addr, 100 * 1000, &handle_);
        if (ret != ESP_OK) {
    ...
    static constexpr uint32_t kMonitorErrorBackoffMs = 250;
    ...
    void monitorTaskLoop() {
        ...
        while (true) {
            ...
            bool read_attempted[kPinCountValue] = {};
            bool read_valid[kPinCountValue] = {};
            bool read_values[kPinCountValue] = {};
            bool had_read_error = false;

            const auto read_once = [&](Pin pin, bool* level) {
                const size_t index = static_cast<size_t>(pin);
                if (index >= kPinCountValue) {
                    had_read_error = true;
                    return false;
                }
                if (!read_attempted[index]) {
                    read_attempted[index] = true;
                    read_valid[index] = readLevel(pin, &read_values[index]) == ESP_OK;
                }
                if (!read_valid[index]) {
                    had_read_error = true;
                    return false;
                }
                *level = read_values[index];
                return true;
            };
            ...
            vTaskDelay(pdMS_TO_TICKS(had_read_error ? kMonitorErrorBackoffMs
                                                    : kMonitorPollMs));
        }
    }
