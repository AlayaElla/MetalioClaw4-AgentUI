...
bool Board::IsNetworkConnected() const {
    return false;
}
...
std::string Board::GetSystemInfoJson() {
    ...
    */
    ...
    json += R"("ota":{)";
    const auto* app_partition = esp_partition_find_first(
        ESP_PARTITION_TYPE_APP, ESP_PARTITION_SUBTYPE_APP_FACTORY, nullptr);
    json += R"("label":")" + std::string(app_partition ? app_partition->label : "factory") + R"(")";
    json += R"(},)";
