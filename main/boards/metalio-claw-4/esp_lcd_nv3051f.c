...
static esp_err_t panel_nv3051f_disp_on_off(esp_lcd_panel_t *panel, bool on_off)
{
    ...
    if (on_off) {
        ESP_RETURN_ON_ERROR(metalio_mipi_dsi_power_set_idle(false), TAG,
                            "resume video stream failed");
        ...
        ESP_RETURN_ON_ERROR(esp_lcd_panel_io_tx_param(io, LCD_CMD_SLPIN, NULL, 0), TAG,
                            "enter sleep failed");
        vTaskDelay(pdMS_TO_TICKS(120));
        ESP_RETURN_ON_ERROR(metalio_mipi_dsi_power_set_idle(true), TAG,
                            "suspend video stream failed");
    }
    return ESP_OK;
}
#endif
