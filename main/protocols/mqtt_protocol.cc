...
bool MqttProtocol::SendAudio(std::unique_ptr<AudioStreamPacket> packet) {
    ...
    *(uint32_t*)&nonce[12] = htonl(++local_sequence_);
    ...
    if (esp_aes_crypt_ctr(&aes_ctx_, packet->payload.size(), &nc_off, (uint8_t*)nonce.data(), stream_block,
...
bool MqttProtocol::RequiresAudioChannelPrime() const {
    return true;
}

bool MqttProtocol::PrimeAudioChannel(std::unique_ptr<AudioStreamPacket> packet) {
    if (packet == nullptr || packet->payload.empty()) {
        ESP_LOGE(TAG, "Missing encoded UDP priming frame");
        return false;
    }
    const bool sent = SendAudio(std::move(packet));
    ESP_LOGI(TAG, "UDP audio return path prime %s", sent ? "sent" : "failed");
    return sent;
}
...
void MqttProtocol::ParseServerHello(const cJSON* root) {
    ...
    const std::string decoded_key = DecodeHexString(key);
    esp_aes_setkey(&aes_ctx_, reinterpret_cast<const unsigned char*>(decoded_key.data()), 128);
    local_sequence_ = 0;
    remote_sequence_ = 0;
