...
/*
 * Provisioning protocol details are maintained in the project documentation
 * and are not embedded in this public repository.
 */
esp_err_t ProvisioningClient::FetchConfiguration() {
    ...
    if (url.length() < 10) {
        ESP_LOGE(TAG, "Provisioning URL is not properly set");
    ...
    if (status_code != 200) {
        ESP_LOGE(TAG, "Failed to fetch provisioning configuration, status code: %d", status_code);
        return status_code;
    }
...
std::string ProvisioningClient::GetActivationPayload() {
    if (!has_serial_number_) {
        return "{}";
    }
...
esp_err_t ProvisioningClient::Activate() {
