...
// descriptor/alignment budget and stay on software while it is unavailable.
...
static bool hw_jpeg_ensure_inited(void) {
    ...
    if (!jpeg_hardware_dma_budget_available()) {
        ESP_LOGW(TAG,
                 "hardware JPEG encoder deferred: internal DMA descriptors unavailable; using software");
        return false;
    }
...
bool image_to_jpeg_software_cb(uint8_t* src, size_t src_len, uint16_t width,
                               uint16_t height, v4l2_pix_fmt_t format,
                               uint8_t quality, jpg_out_cb cb, void* arg) {
    if (src == NULL || src_len == 0 || width == 0 || height == 0 || cb == NULL) {
        return false;
    }
    return encode_with_esp_new_jpeg(src, src_len, width, height, format,
                                    quality, NULL, NULL, cb, arg);
}
