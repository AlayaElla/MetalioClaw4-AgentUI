...
// The JPEG hardware driver allocates two cache-line-aligned DMA2D descriptors
// from INTERNAL|DMA memory before it has a valid codec handle. Match that
// descriptor/alignment budget and defer to esp_new_jpeg while it is unavailable.
static bool jpeg_hardware_dma_budget_available(void) {
    const uint32_t caps = MALLOC_CAP_INTERNAL | MALLOC_CAP_DMA;
    size_t cache_alignment = 0;
    if (esp_cache_get_alignment(caps, &cache_alignment) != ESP_OK ||
        cache_alignment == 0) {
        return false;
    }
    const size_t descriptor_size =
        ((sizeof(dma2d_descriptor_t) + cache_alignment - 1) /
         cache_alignment) *
        cache_alignment;
    const size_t required = descriptor_size * 2;
    return heap_caps_get_largest_free_block(caps) >= descriptor_size &&
           heap_caps_get_free_size(caps) >= required;
}
...
esp_err_t jpeg_to_image_software(const uint8_t* src, size_t src_len,
                                 uint8_t** out, size_t* out_len,
                                 size_t* width, size_t* height,
                                 size_t* stride) {
    if (src == NULL || src_len == 0 || out == NULL || out_len == NULL ||
        width == NULL || height == NULL || stride == NULL) {
        ESP_LOGE(TAG, "Invalid parameters");
        return ESP_ERR_INVALID_ARG;
    }
    return decode_with_new_jpeg(src, src_len, out, out_len, width, height,
                                stride);
}
...
#ifdef CONFIG_XIAOZHI_ENABLE_HARDWARE_JPEG_DECODER
    if (jpeg_hardware_dma_budget_available()) {
        ...
        ESP_LOGW(TAG,
                 "hardware JPEG decoder deferred: internal DMA descriptors unavailable; using software");
    }
    // Fallback to esp_new_jpeg
#endif
    return jpeg_to_image_software(src, src_len, out, out_len, width, height,
                                  stride);
}
